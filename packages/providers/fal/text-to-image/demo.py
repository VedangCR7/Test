...
def confirm_generation(cost_estimate: str) -> bool:
    ...
    confirm = (
        input("💰 Do you want to proceed? This will charge your account (y/N): ")
        .strip()
        .lower()
    )
...
def generate_single_image(generator: FALTextToImageGenerator):
    ...
    try:
        ...
        if model_key in ["seedream", "flux_dev"]:
            negative_prompt = get_user_input(
                "❌ Enter negative prompt (optional, what to avoid)", ""
            )
        ...
        result = generator.generate_image(
            prompt=prompt, model=model_key, negative_prompt=negative_prompt
        )
            ...
            if download != "n":
                try:
                    ...
                    local_path = generator.download_image(
                        result["image_url"], "output", filename
                    )
...
def compare_all_models(generator: FALTextToImageGenerator) -> bool:
    ...
    # Get negative prompt
    negative_prompt = get_user_input(
        "❌ Enter negative prompt (optional, for compatible models)", ""
    )
    if not negative_prompt:
        negative_prompt = None
    ...
    try:
        results = generator.compare_models(
            prompt=prompt, negative_prompt=negative_prompt, output_folder="output"
        )
...
def batch_generate_models(generator: FALTextToImageGenerator) -> bool:
    ...
    if compatible_models:
        negative_prompt = get_user_input(
            f"❌ Enter negative prompt (optional, for {', '.join(compatible_models)})",
            "",
        )
        if not negative_prompt:
            negative_prompt = None
    ...
    try:
        ...
        for model, model_result in results.items():
            if model_result.get("success"):
                ...
                print(
                    f"❌ {model}: FAILED - {model_result.get('error', 'Unknown error')}"
                )
...
def show_model_info(generator: FALTextToImageGenerator):
    ...
    for model_key, info in model_info.items():
        ...
        print(
            f"   Negative Prompts: {'✅' if info['supports_negative_prompt'] else '❌'}"
        )
