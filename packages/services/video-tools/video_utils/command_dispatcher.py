...
class CommandDispatcher(BaseController):
    ...
    def __init__(
        self, input_dir: str = "input", output_dir: str = "output", verbose: bool = True
    ):
    ...
    def _show_main_menu(self) -> bool:
        ...
        if self.verbose:
            ...
            print(
                "Reorganized with class-based architecture for better maintainability."
            )
            print()
    ...
    def _analyze_videos(self) -> bool:
        ...
        analysis_choice = self.get_user_choice(
            "🎯 Select analysis type:", analysis_options
        )
        analysis_map = {
            "1": "description",
            "2": "transcription",
        ...
        for i, video_file in enumerate(video_files, 1):
            ...
            try:
                result = analyze_video_file(
                    video_file, analysis_type, detailed=detailed
                )
                if result:
        ...
        for i, audio_file in enumerate(audio_files, 1):
            ...
            try:
                result = analyze_audio_file(
                    audio_file, analysis_type, detailed=detailed
                )
                if result:
        ...
        for i, image_file in enumerate(image_files, 1):
            ...
            try:
                result = analyze_image_file(
                    image_file, analysis_type, detailed=detailed
                )
                if result:
    ...
    def _comprehensive_analysis(self) -> bool:
        ...
        for i, file_path in enumerate(all_files, 1):
            ...
            try:
                result = analyze_media_comprehensively(
                    file_path, self.output_dir, save_results=True
                )
    ...
    def _transcribe_individual(self) -> bool:
        ...
        use_local = self.get_yes_no(
            "🤖 Use local Whisper model? (otherwise API)", False
        )
        ...
        if use_local:
            ...
            model_choice = self.get_user_choice(
                "🎯 Select model size:", model_options, "6"
            )
            model_map = {
                "1": "tiny",
                "2": "base",
    ...
    def _transcribe_batch(self) -> bool:
        ...
        try:
            results = batch_transcribe_whisper(
                all_files, use_local=use_local, save_results=True
            )
    ...
    def _handle_system_status(self) -> bool:
        ...
        if self.verbose:
            ...
            print(
                f"   Input: {self.input_dir} {'✅' if self.input_dir.exists() else '❌'}"
            )
            print(
                f"   Output: {self.output_dir} {'✅' if self.output_dir.exists() else '❌'}"
            )
            ...
            video_count = (
                len(find_video_files(self.input_dir)) if self.input_dir.exists() else 0
            )
            audio_count = (
                len(find_audio_files(self.input_dir)) if self.input_dir.exists() else 0
            )
            image_count = (
                len(find_image_files(self.input_dir)) if self.input_dir.exists() else 0
            )
...
def main():
    ...
    parser.add_argument(
        "--input-dir", "-i", default="input", help="Input directory (default: input)"
    )
    parser.add_argument(
        "--output-dir",
        "-o",
