    ...
    try:
        ...
        for model_key, model_info in all_models.items():
            ...
            if model_key == "photon":
                ...
                print(
                    f"      • Aspect Ratios: {', '.join(model_info['supported_aspect_ratios'])}"
                )
            else:  # kontext
                print(f"      • Inference Steps: {model_info['inference_steps_range']}")
                print(f"      • Guidance Scale: {model_info['guidance_scale_range']}")
                print(
                    f"      • Resolution Modes: {', '.join(model_info['supported_aspect_ratios'])}"
                )
...
def get_float_input(
    prompt: str, min_val: float, max_val: float, default: float
) -> float:
...
def single_image_demo():
    ...
    try:
        ...
        print("\n📥 Image Input:")
        input_type = (
            input("   Use local file or URL? (local/url) [url]: ").strip().lower()
        )
        ...
        if input_type == "local":
            ...
            image_url = get_user_input(
                "   Enter image URL", "https://picsum.photos/512/512"
            )
        ...
        if model in ["photon", "photon_base"]:
            ...
            strength = get_float_input(
                "   Modification strength (0.0-1.0)", 0.0, 1.0, 0.7
            )
            ...
            print(
                "❌ FLUX Kontext Multi requires multiple images. Use 'Multi-Image Generation' menu option instead."
            )
            return
...
def multi_image_demo():
    ...
    try:
        sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from fal_image_to_image_generator import (
        ...
        image_paths = []
        input_type = (
            input("   Use local files or URLs? (local/url) [url]: ").strip().lower()
        )
        ...
        for i in range(count):
            if input_type == "local":
                ...
                url = get_user_input(
                    f"   Image {i + 1} URL",
                    f"https://picsum.photos/512/512?random={i + 1}",
                )
        ...
        print("\n✏️  Generation:")
        prompt = get_user_input(
            "   Describe the desired generation",
        ...
        while True:
            try:
                choice = input(
                    f"\nChoose aspect ratio (1-{len(KONTEXT_MULTI_ASPECT_RATIOS)}) [5]: "
                ).strip()
                if not choice:
                ...
                if 0 <= index < len(KONTEXT_MULTI_ASPECT_RATIOS):
                    ...
                    print(
                        f"❌ Please choose a number between 1 and {len(KONTEXT_MULTI_ASPECT_RATIOS)}"
                    )
            except ValueError:
                print("❌ Please enter a valid number")
...
def batch_demo():
    ...
    try:
        ...
        for i in range(count):
            ...
            prompt = get_user_input(
                "   Modification prompt", f"Apply artistic style {i + 1}"
            )
...
def model_comparison_demo():
    ...
    try:
        ...
        for model_name, result in results.items():
            ...
            if result["success"]:
                print(
                    f"   ✅ {model_display}: Success ({result['processing_time']:.2f}s)"
                )
                for file_path in result["downloaded_files"]:
                    print(f"      📁 {file_path}")
            else:
                print(
                    f"   ❌ {model_display}: Failed - {result.get('error', 'Unknown')}"
                )
        ...
        if results["photon"]["success"] and results["kontext"]["success"]:
            ...
            faster_model = (
                "Photon Flash" if photon_time < kontext_time else "FLUX Kontext"
            )
