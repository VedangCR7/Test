...
OUTPUT_BUCKET_PATH = os.getenv(
    "OUTPUT_BUCKET_PATH", "gs://your-bucket-name/veo_output/"
)
...
def demo_model_comparison():
    ...
    try:
        video_uri_3 = generate_video_with_veo3_preview(
            project_id=PROJECT_ID, prompt=prompt, output_bucket_path=OUTPUT_BUCKET_PATH
        )
...
def show_configuration():
    ...
    if PROJECT_ID == "your-project-id":
        print(
            "  ⚠️  Using default project ID - update .env file with your actual project ID"
        )
    if OUTPUT_BUCKET_PATH == "gs://your-bucket-name/veo_output/":
        print(
            "  ⚠️  Using default bucket path - update .env file with your actual bucket"
        )
    ...
    print("\nConfiguration Help:")
    print("  📝 To update configuration, edit the .env file in this directory")
    print("  🔧 Run 'python fix_permissions.py' to fix Google Cloud permissions")
    print("  🧪 Run 'python test_veo.py' to test your configuration")
...
def main():
    ...
    while True:
        ...
        if choice < 4:
            continue_choice = (
                input("\n🔄 Would you like to try another demo? (y/n): ")
                .lower()
                .strip()
            )
